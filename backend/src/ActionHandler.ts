...
import { AbstractActionHandler, Block, IndexState } from 'demux';
...
const stateHistoryMaxLength: number = 300;

class ActionHandler extends AbstractActionHandler {
  async handleWithState(handle: (state: any, context?: any) => void): Promise<void> {
    await handle(state);
  ...
  async updateIndexState(stateObj: any, block: Block, isReplay: boolean, handlerVersionName: string, context?: any): Promise<void> {
